    ...
    if cheapest_flights:
        ...
        for idx, flight in enumerate(cheapest_flights):
            with cols[idx]:
                ...
                # Safe booking token logic
                departure_token = flight.get("departure_token", "")
                booking_options = None
                if departure_token:
                    try:
                        params_with_token = {**params, "departure_token": departure_token}
                        ...
                        if idx < len(best_flights_list):
                            booking_options = best_flights_list[idx].get("booking_token")
                    except Exception as e:
                        st.warning(f"Booking info not found for flight #{idx+1}")
                ...
                <div style="border: 2px solid #ddd; border-radius: 10px; padding: 15px; text-align: center;
                    ...
                    <a href="{booking_link}" target="_blank" style="
                        ...
                        color: #fff; background-color: #007bff; text-decoration: none; border-radius: 5px; margin-top: 10px;
                    ">🔗 Book Now</a>
                </div>
                """, unsafe_allow_html=True)
    else:
